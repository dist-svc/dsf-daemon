...
fn test_manager() {
    ...
    let mut mux = MockConnector::<Address, Request, Response, Error, ()>::new();

    let mut dsf = Builder::new(mux.clone()).config(config).build().unwrap();
    ...
    dsf.connect(rpc::ConnectOptions{address: a2, id: None, timeout: Duration::from_secs(10).into()}).wait().unwrap();
    ...
    dsf.store(&id1, vec![p1.clone()],()).wait().unwrap();
    mux.finalise();
    ...
    mux.expect(vec![]);
    let info = dsf.create(rpc::CreateOptions::default()).wait().expect("error creating service");
    mux.finalise();
    ...
    mux.expect(transactions.clone());
    dsf.register(rpc::RegisterOptions{service: rpc::ServiceIdentifier{id: Some(info.id.clone()), index: None}, no_replica: true}).wait().expect("Registration error");
    mux.finalise();

    println!("Publishes data");

    mux.expect(vec![]);
    dsf.publish(rpc::PublishOptions::new(info.id.clone())).wait().expect("publishing error");
    mux.finalise();
