...
impl Engine {
    ...
    pub async fn run(&mut self, running: Arc<AtomicBool>) -> Result<(), Error> {
        ...
        let (wire, mut net) = (self.wire.take().unwrap(), self.net.take().unwrap());

        // Handle received network messages in the engine
        let dsf_net = self.dsf.clone();
        let _engine_net_handle: JoinHandle<Result<(), Error>> = task::spawn(async move {
            while let Some(net_msg) = net.next().await {
                let mut dsf = dsf_net.clone();
                let mut wire = wire.clone();

                task::spawn(async move {
                    // Extract address
                    let address = net_msg.address.clone();

                    // Decode message via wire module (routes responses internally)
                    // Note this internal routing needs to _always_ occur (may not block)
                    let req = match wire.handle_incoming(&net_msg, |id| dsf.find_public_key(id)).await {
                        // Incoming request
                        Ok(Some(v)) => v,
                    ...
                    // TODO: refactor to a request stack with polling
                    let resp = match dsf.handle_net(address, req).await {
                        Ok(v) => v,
                        Err(e) => {
                    ...
                    let resp_data = match wire.encode(DsfMessage::Response(resp)) {
                        Ok(v) => v,
                        Err(e) => {
                    ...
                    // Send response
                    if let Err(e) = net_msg.reply(resp_data.into()).await {
                        error!("error return DSF response")
                    }
                });
            }
        ...
        // Handle received RPC messages in the engine
        let dsf_unix = self.dsf.clone();
        let mut unix_rx = self.unix.take().unwrap();
        let _engine_unix_handle: JoinHandle<Result<(), Error>>  = task::spawn(async move {
            while let Some(rpc_msg) = unix_rx.next().await {
                let mut dsf = dsf_unix.clone();

                task::spawn(async move {
                    if let Err(e) = Self::handle_rpc(&mut dsf, rpc_msg).await {
                        error!("RPC error: {:?}", e);
                    }
                });
            }
        ...
        let mut _tick_timer = stream::interval(Duration::from_secs(1));

        // Handle DSF update ticks
        let dsf_tick = self.dsf.clone();
        let _engine_tick_handle: JoinHandle<Result<(), Error>> = task::spawn(async move {

            while let Some(interval) = update_timer.next().await {
                trace!("engine::tick");
                // TODO: do something
            }
        ...
        futures::try_join!(_engine_net_handle, _engine_unix_handle);
